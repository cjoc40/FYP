...
@Composable
fun MctsStats(navController: NavHostController, viewModel: Connect4ViewModel) {
        ...
        LazyColumn(
            ...
            if (allGameStats.isEmpty()) {
                ...
                items(allGameStats.size) { gameIndex ->
                    ...
                    GameStatsItem(gameStats, gameIndex + 1, viewModel)
                }
            }
        }
...
@Composable
fun GameStatsItem(gameStats: GameStats2, gameNumber: Int, viewModel: Connect4ViewModel) {
    Column(Modifier.padding(8.dp)) {
        if (gameStats.modelUsed == "MCTS") {
            Text(
                "Game #$gameNumber, Algorithm: MCTS, ${gameStats.simulations} Sims",
                fontSize = 16.sp,
        ...
        if (gameStats.modelUsed == "Minimax") {
            Text(
                "Game #$gameNumber, Algorithm: Minimax, Pruning: ${gameStats.PruningEnabled}, Difficulty: ${gameStats.difficulty}",
                fontSize = 16.sp,
                fontWeight = FontWeight.Bold
            )
        }
